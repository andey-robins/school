...
data Table =
      CreateTable TableName [(String, Type)] TablePrime
      | JoinOn Table Table Expression TablePrime
      | SelectFrom [FieldName] TableName TablePrime
      | SelectFromWhere [FieldName] TableName Expression TablePrime
      | Table TableName TablePrime

data TablePrime =
      InsertInto Table Tuple TablePrime
      | Sequence Table TablePrime
     -- | Go Table TablePrime
      | Epsilon
...
<Table> ::= CREATE TABLE <TableName> ([[<FieldName> <Type>]]+{,} ) <TablePrime>
            | INSERT INTO <Table> <Tuple> <TablePrime>
            | SELECT <FieldName> FROM <Table> <TablePrime>
            | SELECT <FieldName> FROM <Table> WHERE <Exp> <TablePrime>
            | <TableName> <TablePrime>
            | ( <Table> ) <TablePrime>

<TablePrime> ::= JOIN ON <Table> <Table> <Expression> <TablePrime>
            | ; <Table> <TablePrime>
            | Epsilon
...
createTab t = do {symbol "CREATE TABLE";
                  ...
                  t2 <- fieldType;
                  symbol ")";
                  return (CreateTable n [(f, t2)] Epsilon)}

tableTab = do n <- tableName
              return (Table n Epsilon)

table' t = insertTab t +++ joinTab t +++ selectFromTab t +++ selectFromWhereTab t +++ createTab t +++ return t
-- don't forget to add goTab back in once you figure out what that does
-- add sequence tab back in again

insertTab t1 = do {symbol "INSERT INTO";
                   t2 <- table';
                   u <- tuple;
                   return (InsertInto t2 u Epsilon)}

joinTab t = do {symbol "JOIN ON";
                t2 <- table' t;
                t3 <- table' t;
                e <- expression;
                return (JoinOn t2 t3 e Epsilon)}

selectFromTab t = do {symbol "SELECT";
                      f <- fieldName;
                      symbol "FROM";
                      n <- tableName;
                      return (SelectFrom [f] n Epsilon)}
...
selectFromWhereTab t = do {symbol "SELECT";
                           ...
                           symbol "WHERE";
                           e <- expression;
                           return (SelectFromWhere [f] n e Epsilon)}

-- sequenceTab t = do {t2 <- table' t;
--                     symbol ";";
--                     t3 <- table' t2;
--                     return (Sequence t2 (table' t3))}
