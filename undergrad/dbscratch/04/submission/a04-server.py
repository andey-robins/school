...
def run_insert(stmt, data):
    ...
    try:
        ...
        cur.execute(stmt, data)
        cur.close()
        return "{"+"\"status\":\"success\",\"id\":\"{}\"".format(data["issue_id"])+"}"
...
def create_issue():
    ...
    if request.method == 'GET':
        ...
        parameters = {}
        for key, val in request.forms.items():  # pylint: disable=no-member
            parameters[key] = [val]

    if not required_param(parameters, ["title", "body"]):
    ...
    severity_id = 1
    if "severity_id" in parameters.keys():
        severity_id = parameters["severity_id"][0]
    state_id = 1
    if "state_id" in parameters.keys():
        state_id = parameters["state_id"][0]
    issue_id = gen_uuid()
    if "issue_id" in parameters.keys():
        issue_id = parameters["issue_id"][0]

    # perform DB operations
    s = run_insert(
        "INSERT INTO i_issue ( id, title, body, severity_id, state_id ) VALUES ( %(issue_id)s, %(title)s, %(body)s, %(severity_id)s, %(state_id)s );", {"issue_id": issue_id, "body": body, "title": title, "severity_id": severity_id, "state_id": state_id})
...
def delete_issue():
    ...
    if not required_param(parameters, ["issue_id"]):
        return "{\"status\":\"missing params\"}"

    s = run_delete('DELETE FROM i_note WHERE issue_id=%(issue_id)s;',
                   {"issue_id": parameters["issue_id"][0]})
    s = run_delete('DELETE FROM i_issue WHERE id=%(issue_id)s;',
                   {"issue_id": parameters["issue_id"][0]})
    db_conn.commit()
    return s
...
def update_issue():
    ...
    if not required_param(parameters, ["issue_id"]) and (required_param(parameters, ["title"]) or required_param(parameters, ["body"])):
        return "{\"status\":\"missing params\",}"

    if "body" in parameters.keys() and "title" in parameters.keys():
        s = run_update('UPDATE i_issue SET title=%(title)s, body=%(body)s WHERE id=%(id)s;', {
                       "id": parameters["issue_id"][0], "body": parameters["body"][0], "title": parameters["title"][0]})
    elif "body" in parameters.keys():
        s = run_update('UPDATE i_issue SET body=%(body)s WHERE id=%(id)s;', {
                       "id": parameters["issue_id"][0], "body": parameters["body"][0]})
    else:
        s = run_update('UPDATE i_issue SET title=%(title)s WHERE id=%(id)s;', {
                       "id": parameters["issue_id"][0], "title": parameters["title"][0]})
...
def get_issue_detail():
    ...
    if not required_param(parameters, ["issue_id"]):
        return "{\"status\":\"missing issue_id param\",}"
    ...
    if issue_data["n_rows"] == 1:
        note_data = run_select_raw(
            "SELECT * FROM i_note WHERE issue_id=%(issue_id)s ORDER BY seq;", {"issue_id": issue_id})
    ...
    s = json.dumps(issue_result, indent=1, default=default)
    return "{" + "\"status\":\"success\", \"n_rows\": {}, \"data\": {}".format(num_rows, s) + "}"
...
def create_note():
    ...
    if not required_param(parameters, ["issue_id", "title", "body"]):
        return "{\"status\":\"missing issue_id param\",}"

    s = run_insert("INSERT INTO i_note ( issue_id, title, body ) VALUES ( %(issue_id)s, %(title)s, %(body)s );", {
                   "issue_id": parameters["issue_id"][0], "title": parameters["title"][0], "body": parameters["body"][0]})
    db_conn.commit()
    return s
...
def delete_note():
    ...
    if not required_param(parameters, ["issue_id", "note_id"]):
        return "{\"status\":\"missing param\",}"

    s = run_delete('DELETE FROM i_note WHERE id=%(note_id)s AND issue_id=%(issue_id)s', {
                   "note_id": parameters["note_id"][0], "issue_id": parameters["issue_id"][0]})
    db_conn.commit()
    return s
...
def upd_severity():
    ...
    if not required_param(parameters, ["issue_id", "severity_id"]):
        return "{\"status\":\"missing issue_id param\",}"

    s = run_update('UPDATE i_issue SET severity_id=%(severity_id)s WHERE id=%(issue_id)s', {
                   "severity_id": parameters["severity_id"][0], "issue_id": parameters["issue_id"][0]})
    db_conn.commit()
    return s
...
def delete_note_TODO():
    response.content_type = "application/json"
    dict = parse_qs(request.query_string)
    if not required_param(dict, ["note_id"]):
        return
    note_id = dict["note_id"][0].lower()
...
def update_note():
    ...
    if not required_param(dict, ["note_id", "title", "body"]):
        return
    note_id = dict["note_id"][0].lower()
    title = dict["title"][0].lower()
    body = dict["body"][0].lower()
    return run_update("UPDATE i_note SET title = %(title)s, body = %(body)s WHERE id = %(id)s", {"id": note_id, "title": title, "body": body})
...
def issue_get():
    if request.method == 'GET':
        ...
        if "__method__" in dict:
            method = dict["__method__"][0].lower()
            if method == "post":
                return create_issue()
            elif method == "delete":
                return delete_issue()
...
if __name__ == '__main__':
    try:
        ...
        app.run(port=xport, host=xhost, debug=True)
